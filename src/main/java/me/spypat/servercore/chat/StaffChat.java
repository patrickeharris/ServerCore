...
public class StaffChat implements CommandExecutor{
	...
	public CommandResult execute(CommandSource source, CommandContext context) throws CommandException {
		if(!(source instanceof Player)){
			source.sendMessage(Text.of("You Must Be A Player To Enter Staff Chat!"));
			return CommandResult.builder().successCount(0).build();
		}
		Player p = (Player) source;
		if(!PermPlayer.hasPerm(p, "staff.chat")){
		...
		if(staffchat.contains(p.getName())){
			...
			p.sendMessage(Text.of("You Have Exited Staff Chat!"));
			return CommandResult.builder().successCount(1).build();
		}
		staffchat.add(p.getName());
		p.sendMessage(Text.of("You Have Entered Staff Chat!"));
